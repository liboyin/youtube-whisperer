...
if __name__ == '__main__':
    input_file_path = prepare_input_file(DEFAULT_HOME_DIR / 'test.mp4')
    segments_file_path = prepare_output_file(input_file_path.with_suffix('.txt'))
    srt_file_path = prepare_output_file(input_file_path.with_suffix('.srt'))
    segments_generator = transcribe_waveform_with_default_model(load_waveform_from_file(input_file_path))
    segments_generator = duplicate_segments_to_file(segments_generator, segments_file_path)
    srt_blocks_generator = yield_deduplicated_srt_blocks(yield_srt_blocks_from_segments(segments_generator))
    srt_file_path.write_text(convert_srt_blocks_to_str(srt_blocks_generator))
