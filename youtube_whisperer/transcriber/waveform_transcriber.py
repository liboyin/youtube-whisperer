...
def transcribe_file_with_default_model(input_file_path: Path, output_file_path: Path | None = None, overwrite: OverwriteMode = OverwriteMode.PROMPT, **kwargs) -> Path:
    """
    ...
    segments_generator = transcribe_waveform_with_default_model(load_waveform_from_file(input_file_path), **kwargs)
...
def main() -> None:
    ...
    parser.add_argument("-o", "--overwrite", choices=OverwriteMode.values(), default=OverwriteMode.PROMPT.value, help="Whether to overwrite existing Segment files. Defaults to `OverwriteMode.PROMPT`.")
