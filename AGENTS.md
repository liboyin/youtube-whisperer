...
- Use focused checks during development. The final code, test, or configuration candidate MUST pass the coverage gate (`./check-coverage.sh`), `mypy youtube_whisperer`, and `ruff check .`. Inspect per-file coverage and warnings; resolve new source warnings and explain accepted tooling warnings.
