...
#### B1 — The mandated review harness cannot be installed in the devcontainer

**Validated — install attempt on 2026-09-11.** `claude plugin marketplace add openai/codex-plugin-cc` succeeds, but `claude plugin install codex@openai-codex` fails with `EISDIR` while renaming its temporary file onto `/home/ubuntu/.claude/plugins/installed_plugins.json`. That path is a bind mount declared in the devcontainer; the host source does not exist, so Docker created it as a root-owned directory, and the CLI cannot replace a directory with its JSON file. **Paths:** [devcontainer.json](.devcontainer/devcontainer.json), [postCreateCommand.sh](.devcontainer/postCreateCommand.sh). **Direction:** ensure the host path exists as a JSON file before the container starts, and consider seeding it in `postCreateCommand.sh`, which already compensates for the same Docker behavior one directory up. **Acceptance:** after a rebuild, `claude plugin list` shows the plugin, `/codex:setup` reports Codex ready, and the review skill dispatches without manual steps. **Impact while open:** AGENTS requires adversarial review before committing any non-trivial change, and the skill correctly refuses to substitute a reviewer, so such changes cannot be committed as specified.
