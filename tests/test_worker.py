...
def test_process_queue_url_with_no_transcript(mocker, mock_redis):
    ...
    mock_transcribe.assert_called_once_with([Path('/path/to/video.mp4')], task['language'], overwrite=OverwriteMode.NEVER)
...
def test_process_queue_local_file(mocker, mock_redis):
    ...
    mock_transcribe.assert_called_once_with([Path(task['source'])], task['language'], overwrite=OverwriteMode.NEVER)
